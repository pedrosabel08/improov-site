<header class="site-header" data-site-header>
  <a class="brand" href="<?= escape(base_url()) ?>" aria-label="Improov — Início" data-i18n-aria="nav.home">
    <img src="<?= escape(asset('assets/IMPROOV_TOP(black).gif')) ?>" alt="Improov" id="gif-header">
  </a>
  ...
  <div class="language-switcher" role="group" aria-label="Selecionar idioma" data-i18n-aria="language.label">
    <?php foreach (['pt-BR' => 'PT', 'en' => 'EN', 'es' => 'ES'] as $languageCode => $label): ?><a href="<?= escape(base_url($meta['path'], $languageCode)) ?>" hreflang="<?= escape($languageCode) ?>" lang="<?= escape($languageCode) ?>" data-language="<?= escape($languageCode) ?>" aria-current="<?= current_language() === $languageCode ? 'true' : 'false' ?>"><?= escape($label) ?></a><?php endforeach; ?>
  </div>
